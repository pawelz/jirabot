...
JIRA bot is IM bot for Atlassian JIRA. It allows to integrate JIRA with IRC.
It should be easy to integrate it with any other IM protocol supporte by ekg2
(Jabber, GG, ICQ, ...).

Requirements
============

JIRA bot is python script for ekg2, so obviously you need ekg2 with python
...
* copy jirabot.py to `~/.ekg2/scripts/jirabot.py` (create this directory unless
  it already exists)
* run ekg2 inside screen session
...
* `jirabot:password`: username of bot user in JIRA
...
JIRA bot scans jirabot:sigdir directory. It excpects files containing subject
of mail sent by JIRA.
